...
(async () => {
  ...
  page1.on('pageerror', err => console.log('PAGE 1 ERROR:', err.message, err.stack));
  page2.on('pageerror', err => console.log('PAGE 2 ERROR:', err.message, err.stack));
  ...
  try {
    ...
    await page1.evaluate(() => document.querySelector('input.google-real-checkbox').click());
    ...
    await page1.waitForSelector('.google-error-alert', { timeout: 5000 });
    const errorText = await page1.locator('.google-error-alert span').nth(1).innerText();
    ...
    await page1.click('button[type="submit"]');
    await page1.waitForSelector('.google-error-alert', { timeout: 5000 });
    const loginErrorText = await page1.locator('.google-error-alert span').nth(1).innerText();
    console.log(`Incorrect password error shown: "${loginErrorText}"`);
    if (!loginErrorText.toLowerCase().includes('credentials') && !loginErrorText.toLowerCase().includes('password')) {
    ...
    await page1.click('button[type="submit"]');
    await page1.waitForSelector('.google-error-alert', { timeout: 5000 });
    const nonexistentErrorText = await page1.locator('.google-error-alert span').nth(1).innerText();
    console.log(`Nonexistent user error shown: "${nonexistentErrorText}"`);
    if (!nonexistentErrorText.toLowerCase().includes('credentials') && !nonexistentErrorText.toLowerCase().includes('not found')) {
    ...
    await page2.fill('#password', 'usertwo1234');
    await page2.fill('#confirmPassword', 'usertwo1234');
    await page2.evaluate(() => document.querySelector('input.google-real-checkbox').click());
    ...
    const participantNames1 = await page1.$$eval('.participant-username-text', el => el.map(e => e.innerText));
    ...
    await page1.fill('.composer-input-field', 'Hello from User 1!');
    await page1.click('.composer-send-btn');
    ...
    await page2.waitForSelector('text=Hello from User 1!', { timeout: 10000 });
    const messageTexts = await page2.$$eval('.chat-message-bubble p', el => el.map(e => e.innerText));
    ...
    // Reply from User 2
    await page2.fill('.composer-input-field', 'Hi User 1!');
    await page2.click('.composer-send-btn');
    console.log('User Two sent chat message: "Hi User 1!"');

    // Verify reply received on User 1
    await page1.waitForSelector('text=Hi User 1!', { timeout: 10000 });
    const messageTexts1 = await page1.$$eval('.chat-message-bubble p', el => el.map(e => e.innerText));
    ...
    const containerHtml = await page2.innerHTML('.google-room-grid-container');
    console.log('CONTAINER HTML ON PAGE 2:', containerHtml);
